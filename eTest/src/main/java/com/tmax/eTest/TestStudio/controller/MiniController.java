...
public class MiniController {
	...
	public ResponseEntity<List<MiniProblemListDTO>> MiniProblemList(
		...
		try {
			Page<TestProblem> query = miniRepository.searchMiniProblems(partId, keyword, order, orderOption,
					PageRequest.of(pageNo, pageCount));
			List<MiniProblemListDTO> body = query.getContent().stream()
					.map(m -> new MiniProblemListDTO(m.getProbID(),
							ObjectUtils.isEmpty(m.getPart()) ? null : m.getPart().getPartName(), m.getProblem().getDifficulty(),
							m.getSubject(), m.getStatus(), m.getProblem().getQuestion(), m.getProblem().getEditDate()))
