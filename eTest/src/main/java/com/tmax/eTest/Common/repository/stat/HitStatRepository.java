...
public interface HitStatRepository extends JpaRepository<HitStat, Long> {
  ...
  Integer updateHit(@Param("statDate") java.util.Date date, @Param("videoHit") Long videoHit,
      @Param("bookHit") Long bookHit, @Param("wikiHit") Long wikiHit, @Param("articleHit") Long articleHit);

  Boolean existsByStatDate(java.util.Date date);
}
