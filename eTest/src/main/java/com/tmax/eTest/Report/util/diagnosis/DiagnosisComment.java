...
//		};
		...
		//investRiskComment[investRiskComIdx] + " ";
...
//				"투자원칙 정립이 필요한 투자자"};
		...
		profileCommentInfo.put("main", CommentMapper.getComment(Type.INVEST_PROFILE_MAIN, profileIdx));
		profileCommentInfo.put("detail", "");
		profileCommentInfo.put("score", stretchProfileScore);
		profileCommentInfo.put("detailScoreList", profileDetailScore);
		
		tracingCommentInfo.put("name", "투자원칙");
		tracingCommentInfo.put("main", CommentMapper.getComment(Type.INVEST_TRACING_MAIN, tracingIdx));
		tracingCommentInfo.put("detail", "");
