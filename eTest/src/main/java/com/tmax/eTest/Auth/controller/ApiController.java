...
import com.tmax.eTest.Auth.dto.DuplicateCheckDto;
...
public class ApiController {
    ...
    public CMRespDto<?> jwtCreate(@RequestBody Map<String, Object> data) {
        ...
        Optional<UserMaster> userEntity = userRepository.findByProviderIdAndProvider((String) data.get("providerId"),
                AuthProvider.valueOf((String) data.get("provider")));
        if (userEntity.isPresent()) {
            System.out.println("기존에 로그인 했던 유저입니다");
            String jwtToken = JWT.create().withSubject(userEntity.get().getEmail())
                    .withExpiresAt(new Date(System.currentTimeMillis() + JwtProperties.EXPIRATION_TIME))
                    .withClaim("userUuid", userEntity.get().getUserUuid())
                    .withClaim("email", userEntity.get().getEmail()).sign(Algorithm.HMAC512(JwtProperties.SECRET));
    ...
    public CMRespDto<?> signup(@RequestBody SignUpRequestDto signUpRequestDto) {
        ...
        String jwtToken = JWT.create().withSubject(userMaster.getEmail())
                .withExpiresAt(new Date(System.currentTimeMillis() + JwtProperties.EXPIRATION_TIME))
                .withClaim("userUuid", userMaster.getUserUuid()).withClaim("email", userMaster.getEmail())
