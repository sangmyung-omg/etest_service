...
import com.tmax.eTest.dao.CurriculumRepository;
import com.tmax.eTest.dao.DemoChapterPartMapper;
import com.tmax.eTest.dao.ProblemDemoDAO;
import com.tmax.eTest.dao.UkDAO;
import com.tmax.eTest.dao.UkRepository;
import com.tmax.eTest.dao.UserKnowledgeDAO;
import com.tmax.eTest.dao.UserKnowledgeRepository;
import com.tmax.eTest.model.ChapterMasteryDTO;
import com.tmax.eTest.model.UKDTO;
import com.tmax.eTest.model.UKMasteryDTO;
...
//		System.out.println(chapterIdList);
		...
		List<UkDAO> ukInfoList = (List<UkDAO>) ukRepository.findAllById(ukIdList);
//		System.out.println("ukInfoList : " + ukInfoList.size());
		...
		for (UkDAO dao : ukInfoList) {
			if (!ukChapterMap.containsKey(dao.getCurriculumDao().getChapterId())) {
				List<String> l = new ArrayList<String>();
				l.add(dao.getUkName());
				ukChapterMap.put(dao.getCurriculumDao().getChapterId(), l);
			} else {
				ukChapterMap.get(dao.getCurriculumDao().getChapterId()).add(dao.getUkName());
			}
		}
...
//		logger.info("currIdList : " + currIdList);
		...
		for (String currId : ukChapterMap.keySet()) {
			...
			List<UserKnowledgeDAO> ukMasteryForChapters = (List<UserKnowledgeDAO>) userKnowledgeRepository.findAllByUserAndChapter(userId, currId);
			...
			int flag = 0;
			for (UserKnowledgeDAO dao : ukMasteryForChapters) {
...
//						+ dao.getUkDao().getCurriculumDao().getChapterId() + ", " + dao.getUkDao().getCurriculumDao().getChapter());
				...
				if (flag == 0) {
					...
					for (String key : chapterPartList.keySet()) {
						if (chapterPartList.get(key).contains(dao.getUkDao().getCurriculumDao().getChapterId())) {
							template.setPart(key);
						}
					}
					// chapterName
					template.setChapterName(dao.getUkDao().getCurriculumDao().getChapter());
					flag += 1;
				}
				
				// ukList
				template.getUkList().add(dao.getUkDao().getUkName());
				
				// masteryList
				template.getMasteryList().add(dao.getUkMastery());
				
				// 푼 문제의 UK들이 있으면 순서를 앞으로 가저오기.
				if (solved_uk.contains(dao.getUkDao().getUkName())) {
					int flag2 = 0;
