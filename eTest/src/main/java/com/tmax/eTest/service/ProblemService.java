...
import com.tmax.eTest.dao.ProblemDemoRepository;
import com.tmax.eTest.dao.ProblemSetDemoDAO;
import com.tmax.eTest.dao.ProblemSetRepository;
...
public class ProblemService {
	...
	public List<Map<String, String>> getNextProblemSet(String userId, String diagType, String part, String is_adaptive){
		...
		UserDAO dao = userService.getUserInfo(userId);
		...
		ProblemSetDemoDAO resultDAO;
		List<ProblemSetDemoDAO> queryResult = problemSetRepository.findAllByChapter(selected_chapter);
		...
		prob_list.add(resultDAO.getProb1Uuid());
		prob_list.add(resultDAO.getProb2Uuid());
		prob_list.add(resultDAO.getProb3Uuid());
//		System.out.println("selected problem set : " + resultDAO.toString() + "\n3 problem infos for the problem set : " + problemRepository.findAllById(prob_list));
		...
		List<ProblemDemoDAO> pdDaoList = (List<ProblemDemoDAO>) problemRepository.findAllById(prob_list);
		if (pdDaoList == null) {
		...
		// 정렬
		Collections.sort(pdDaoList, new Comparator<ProblemDemoDAO>() {
			@Override
			public int compare(ProblemDemoDAO pddao1, ProblemDemoDAO pddao2) {
				return pddao2.getDifficulty().compareTo(pddao1.getDifficulty());
			}
		});
		
		// 각 문제 정보를 output format에 맞게 넣어줌
		for (ProblemDemoDAO pddao : pdDaoList) {
