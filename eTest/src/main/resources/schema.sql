...
(
    ...
    REPORT_TEXT    VARCHAR2(1024)    NULL, 
    CONSTRAINT PK_ERROR_REPORT PRIMARY KEY (ERROR_ID)
);

CREATE SEQUENCE ERROR_SEQ START WITH 1 INCREMENT BY 1;

COMMENT ON COLUMN ERROR_REPORT.ERROR_ID IS '에러 ID';

COMMENT ON COLUMN ERROR_REPORT.PROB_ID IS '문제 ID';

COMMENT ON COLUMN ERROR_REPORT.USER_UUID IS 'USER_UUID';

COMMENT ON COLUMN ERROR_REPORT.REPORT_TYPE IS '신고 유형';

COMMENT ON COLUMN ERROR_REPORT.REPORT_TEXT IS '신고 내용';

ALTER TABLE ERROR_REPORT
    ADD CONSTRAINT FK_ERROR_REPORT_PROB_ID_PROBLE FOREIGN KEY (PROB_ID)
        REFERENCES PROBLEM (PROB_ID);

ALTER TABLE ERROR_REPORT
    ADD CONSTRAINT FK_ERROR_REPORT_USER_UUID_USER FOREIGN KEY (USER_UUID)
        REFERENCES USER_MASTER (USER_UUID);
